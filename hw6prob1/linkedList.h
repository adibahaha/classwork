...
public:
    ...
    bool divideAt(linkedListType<Type>& secondList, const Type& item);
    //Divide the list at the node with info item into two sublist
    void initializeList(); 
...
template <class Type>
bool linkedListType<Type>::divideAt(linkedListType<Type>& secondList, const Type& item)
{
    int old_count = 0;
    secondList.destroyList();
    nodeType<Type> *current  = first, *trailCurrent = nullptr;
    ...
    else
    {
        ...
        secondList.last = last;
        secondList.count = count - old_count;
        secondList.last = trailCurrent;
        secondList.last->link = nullptr;
        count = old_count;
    }
    return true;
}
